...
import { useEffect } from 'react'
import { themeClasses } from '@/styles'
import { TeamOutlined, DatabaseOutlined, ApartmentOutlined, ClockCircleOutlined, CheckCircleOutlined, FileSearchOutlined } from '@ant-design/icons'
import { useUsers } from '@/features/admin/hooks/useUsers'
...
export default function AdminDashboard() {
  ...
  const tasksPending = allAssignments.filter(a => 
    ['ASSIGNED', 'IN_PROGRESS', 'IN_EDITING'].includes((a.status || '').toUpperCase())
  ).length
  ...
  const tasksNeedingReview = allAssignments.filter(a => 
    (a.status || '').toUpperCase() === 'REVIEWING'
  ).length

...
export default function GuidelineSection({ guideline }: { guideline?: string }) {
  ...
  return (
    <div
      className={`${themeClasses.backgrounds.card} border ${themeClasses.borders.violet20} rounded-2xl overflow-hidden transition-all duration-300`}
    >
      <button
      ...
      >
        ...
        <span
          className={`material-symbols-outlined text-gray-400 text-[20px] transition-transform duration-300 ${expanded ? 'rotate-180' : ''}`}
        >
          expand_more
        </span>
      </button>

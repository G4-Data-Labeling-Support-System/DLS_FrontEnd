...
const ReviewerWorkspacePage: React.FC = () => {
    ...
    const getNextItemId = useCallback((currentId: string): string | null => {
    ...
    const prefetchNextItem = useCallback(async (currentId: string) => {
    ...
    const handleReviewDecision = useCallback(async (status: 'approved' | 'rejected', reason?: string, feedback?: string[]) => {
        if (!selectedId) return;
    ...
    useEffect(() => {
        const handleKeyPress = (e: KeyboardEvent) => {
            ...
            if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement || isApproveModalOpen || isRejectModalOpen) {
                return;
            }
        ...
        <div className="h-[calc(100vh-160px)]">
            <div className="flex gap-6 h-full">
                ...
                <div className="relative w-80 shrink-0 flex-none">
                    <div className={`h-full transition-all duration-300 ${loadingItems ? 'opacity-60' : 'opacity-100'}`}>
                        {loadingItems && (
                ...
                <div className="relative w-80 shrink-0 flex-none">
                    <div className={`h-full transition-all duration-300 ${loadingDetail ? 'opacity-60' : 'opacity-100'}`}>
                        {loadingDetail && (
                            <div className="absolute inset-0 bg-gradient-to-br from-purple-900/30 via-blue-900/20 to-transparent z-50 flex items-center justify-center backdrop-blur-sm rounded-2xl border border-purple-500/40 shadow-2xl shadow-purple-500/20">
                                <Spin
                                    indicator={<LoadingOutlined className="text-3xl text-purple-400" spin />}
                                />
                            </div>
                        )}
                        <ReviewDetailPanel

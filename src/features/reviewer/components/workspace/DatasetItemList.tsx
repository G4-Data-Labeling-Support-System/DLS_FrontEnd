    ...
    >
      ...
      <div className="p-4 border-b border-gray-800 space-y-3 shrink-0">
        <div className="flex justify-between items-center">
          ...
          <Badge
            count={items.length}
            style={{ backgroundColor: '#2b2639', color: '#a78bfa', boxShadow: 'none' }}
          />
        </div>
        <Input
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
        {items.map((item) => (
          ...
          >
            {/* Thumbnail */}
            <div
              className={`w-10 h-10 rounded-lg overflow-hidden shrink-0 border ${selectedId === item.id ? 'border-violet-500/50' : 'border-gray-700'}`}
            >
            ...
            <div className="flex-1 min-w-0">
              <Text
                className={`block text-xs font-medium truncate mb-0.5 ${selectedId === item.id ? 'text-violet-200' : 'text-gray-300 group-hover:text-white'}`}
              >
                {item.filename}
              </Text>
              <div className="flex items-center gap-1.5">
                {item.status === 'approved' && (
                  <CheckCircleFilled className="text-[10px] text-green-500" />
                )}
                {item.status === 'rejected' && (
                  <CloseCircleFilled className="text-[10px] text-red-500" />
                )}
                {item.status === 'pending' && (
                  <ClockCircleFilled className="text-[10px] text-yellow-500" />
                )}

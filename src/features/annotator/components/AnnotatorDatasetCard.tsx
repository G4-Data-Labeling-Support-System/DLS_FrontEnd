...
export default function AnnotatorDatasetCard({ projectId }: AnnotatorDatasetCardProps) {
    ...
    return (
        <div className={`glass-panel border ${themeClasses.borders.violet10} rounded-2xl p-6 shadow-xl relative overflow-hidden flex flex-col h-full`}>
            ...
            <div className="relative z-10 flex flex-col h-full">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-mono text-blue-400 tracking-widest uppercase">
                            Datasets
                        </h3>
                    </div>
                ...
                {filteredDatasets.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-gray-500 text-sm italic">
                            {searchTerm ? `No datasets matching "${searchTerm}"` : 'No datasets associated with this project'}
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        {totalPages > 1 && (
                            <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/5">
                                <span className="text-[11px] text-gray-500 font-medium">
                                    Showing <span className="text-gray-300">{startIndex + 1}</span> - <span className="text-gray-300">{Math.min(startIndex + itemsPerPage, filteredDatasets.length)}</span> of <span className="text-gray-300">{filteredDatasets.length}</span> datasets
                                </span>
                                <div className="flex items-center gap-2">
                                    <button
